...
def get_share_link(realty, lang) -> str:
    message_text = get_text('get_text_info_for_share', lang).replace('{bot_name}', settings.bot.NAME)

    encoded_share_message_text = quote(message_text)
    encoded_share_target_url = quote(f"https://t.me/{settings.bot.NAME}?start={realty.id}", safe='')
...
def get_share_link_to_bot(lang) -> str:
    message_text = get_text('get_text_info_for_share_to_bot', lang).replace('{bot_name}', settings.bot.NAME)

    encoded_share_message_text = quote(message_text)
    encoded_share_target_url = quote(f"https://t.me/{settings.bot.NAME}", safe='')
